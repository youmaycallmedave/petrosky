    ...
    ScrollTrigger.create({
      ...
      onEnter: () => {
        gsap.to(lines.slice(0, config.lines), {
          ...
          ease: 'power1.out',
          onUpdate: function() {
            const filled = lines.slice(0, config.lines).filter(line =>
              line.style.backgroundColor === config.color ||
              line.style.backgroundColor === "rgb(249, 66, 38)" ||
              line.style.backgroundColor === "rgb(232, 192, 31)" ||
              line.style.backgroundColor === "rgb(245, 246, 249)"
            ).length;
